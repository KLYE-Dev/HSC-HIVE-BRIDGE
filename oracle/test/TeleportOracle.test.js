...
const gloryHoleDeployer = require('./GloryHoleDeployer.js');
...
describe('teleport ERC20 tokens', () => {
    ...
    beforeEach(async () => {
        ...
        gloryHoleContract = await gloryHoleDeployer({
        ...
        gloryHoleContract.should.not.equal(null);
    ...
    it('GloryHole is opened', async () => {
        // Check BlackHole is not closed
        gloryHoleContract.methods.closed().call({ from: identities[0].address }).should.eventually.be.false;
    });

    it('teloportToAccount', async () => {
        let count = 0;

        wormHole({ gloryHole: gloryHoleContract, onData: () => count++ });

        for (let i = 0; i < identitiesCount; i++) {
            let amount = await erc20Contract.methods.balanceOf(identities[i].address).call({ from: identities[i].address , gas: 3000000});
            await erc20Contract.methods.approve(gloryHoleContract.options.address, amount).send({ from: identities[i].address , gas: 3000000});
            const allowed = await erc20Contract.methods.allowance(identities[i].address, gloryHoleContract.options.address).call();
            allowed.should.be.equal(amount);
            await gloryHoleContract.methods.teleport("te.mgr5ymass").send({ from: identities[i].address , gas: 3000000});
    ...
    it('wormhole gets past teleportations', async () => {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            await erc20Contract.methods.approve(gloryHoleContract.options.address, tokenBalance).send({ from: identities[i].address , gas: 3000000});
            const allowed = await erc20Contract.methods.allowance(identities[i].address, gloryHoleContract.options.address).call();
